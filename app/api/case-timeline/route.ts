...
- After the filing member removes or amends the marker, a CRITICAL step is to obtain independent CIFAS verification — a confirmation direct from CIFAS (not just from the filing member) that the record is clear. This protects the user and gives them hard evidence to show the employer. Note: a Monese response may contain both an initial acknowledgment AND a marker removal confirmation in the same email — if the removal is confirmed (with a removal date), treat the filing member track as resolved and make CIFAS verification the active next step on that track.
